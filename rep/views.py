...
class ReporteMatricula(PermissionRequiredMixin, TemplateView):
    ...
    def post(self, request):
        """ metodo para ejecutar el metodo post y descargue
                el archivo de excel"""
        id_es = request.POST.get('id_est')
        if len(id_es) == 0:
            return redirect("rep:reporte_matricula")
        estudiante = Persona.objects.filter(id_persona=id_es)
        return excel_matricula(estudiante)


# Metodos para filtro de información
def con_jor(request):
    # Consultar jornada
    var = list()
...
class ReporteProfesor(PermissionRequiredMixin, TemplateView):
    ...
    def post(self, request):
        ...
        return excel_profesor(profesor)


"""Funciones para generar Excel"""


def excel_profesor(n_prof):
    ...
    for prof in n_prof:
        # Insertar la imagen del profesor
        img = Image(prof.imagen)
        img.width = 100
        img.height = 150
        ws.add_image(img, "B3")
...
def excel_matricula(estu):
    ...
    for est in estu:
        ...
        ws['A1'] = 'Matrícula'
        ws['B3'].alignment = Alignment(horizontal="center", vertical="center")
        ws['B3'].font = Font(name='times new roman', size=14, bold=True)
        ws['B3'] = est.nombres
    # Sección para fusionar campos
    ws.merge_cells('A1:I1')
    # Tamaño de las Filas
